...
// CDN — avoids Vite public/ transform issues
const GSAP_CDN = 'https://cdn.jsdelivr.net/npm/gsap@3.12.5/dist/gsap.min.js';
const MP_CDN = 'https://cdn.jsdelivr.net/npm/gsap@3.12.5/dist/MotionPathPlugin.min.js';

const ISLAND_SVG = `<svg viewBox="60 280 360 240" xmlns="http://www.w3.org/2000/svg" fill-rule="evenodd" clip-rule="evenodd" stroke-linejoin="round" stroke-miterlimit="2" preserveAspectRatio="xMidYMid meet" style="width:100%;height:100%;overflow:visible">
<defs>
...
<g id="scene">
    <mask id="fish-mask"><rect x="0" y="291" width="230" height="122" style="fill:white;"></rect></mask>
    <mask id="leaves">
    ...
    <mask id="water-mask"><path id="water-mask" d="M357.315,457.852C357.437,447.646 336.649,432.68 329.793,430.146C322.936,427.612 314.374,428.513 238.56,428.513C162.746,428.513 145.106,431.336 137.885,433.152C120.949,437.411 109.195,447.259 109.195,458.368C109.195,465.163 130.018,472.394 230.463,472.394C330.908,472.394 357.193,468.059 357.315,457.852Z" style="fill:white;"></path></mask>
    ...
    <g mask="url(#fish-mask)"><path mask="url(#fish-mask)" id="fish" d="M50.84 379.73a.82.82 0 001.19 1.08c4.62-3.66 8.38-6.75 12.4-6.27 4.93.6 7.64 4.13 11.17 7.58 3.54 3.46 12.51 5.33 15.32 1.09 2.81-4.25.94-12.5-3.43-15.8-4.1-3.11-15.71-5.28-23.96-.09a40.6 40.6 0 00-12.7 12.41z" fill="#0793b8"></path></g>
...
export default function FloatingIsland() {
  ...
  useEffect(() => {
    ...
    loadScript(GSAP_CDN)
      .then(() => loadScript(MP_CDN))
      .then(() => {
        // @ts-ignore
        const gsapLib: any = (window as any).gsap;
        // @ts-ignore
        const MotionPathPlugin: any = (window as any).MotionPathPlugin;

        if (!gsapLib || !MotionPathPlugin) {
          console.error('GSAP or MotionPathPlugin not loaded', typeof gsapLib, typeof MotionPathPlugin);
          return;
        }

        console.log('GSAP loaded successfully, version:', gsapLib.version);
        const t = gsapLib;
        t.registerPlugin(MotionPathPlugin);

        // island floating
        t.to("#whole-island", {
          transformOrigin: "bottom center", y: -15, rotation: 1,
          duration: 1, ease: "sine.inOut", yoyo: !0, repeat: -1
        });
        // tree sway
        t.fromTo("#tree", { transformOrigin: "bottom center", rotation: -6 },
          { transformOrigin: "bottom center", rotation: 5, duration: 2, ease: "sine.inOut", yoyo: !0, repeat: -1 });
        // leaves
        t.to("#leaf1", { transformOrigin: "center right", y: -3, duration: 1, ease: "sine.inOut", yoyo: !0, repeat: -1 });
        t.fromTo("#leaf2", { transformOrigin: "bottom right", rotation: 3 },
          { transformOrigin: "bottom right", rotation: -4, x: -3, y: -3, duration: 1, ease: "sine.inOut", yoyo: !0, repeat: -1 });
        t.to("#leaf3", { transformOrigin: "bottom center", rotation: -6, duration: 1, ease: "sine.inOut", yoyo: !0, repeat: -1 });
        t.to("#leaf4", { transformOrigin: "bottom left", rotation: -6, y: -3, duration: 1, ease: "sine.inOut", yoyo: !0, repeat: -1 });
        t.to("#leaf5", { transformOrigin: "top left", y: -3, duration: 1, ease: "sine.inOut", yoyo: !0, repeat: -1 });
        // water circles
        t.to("#water-circle1", { transformOrigin: "center center", scaleX: 1.2, duration: 1, ease: "sine.inOut", yoyo: !0, repeat: -1 });
        t.to("#water-circle2", { transformOrigin: "center center", scaleX: 0.8, duration: 1, ease: "sine.inOut", yoyo: !0, repeat: -1, delay: -0.5 });
        // waves
        t.fromTo("#tri-wave1", { x: -60 }, { x: 20, duration: 6, repeat: -1, ease: "none" });
        t.fromTo("#tri-wave2", { x: -10 }, { x: 50, duration: 6, repeat: -1, ease: "none" });
        t.fromTo("#tri-wave1>path, #tri-wave2>path", { scaleY: 0 },
          { scaleY: 1, duration: 1, repeat: -1, yoyo: !0, transformOrigin: "bottom center" });
        t.fromTo("#sine-wave-group *", { x: 0 }, { x: 75, repeat: -1, duration: 2, ease: "none" });
        t.fromTo("#sine-wave-group *", { scaleY: 0.8, transformOrigin: "bottom center" },
          { scaleY: 1.2, transformOrigin: "bottom center", repeat: -1, duration: 1, yoyo: !0, ease: "sine.inOut" });
        // fish
        t.set("#fish-path", { scaleY: 1.3, scaleX: 1.3, transformOrigin: "bottom left" });
        t.to("#fish", {
          duration: 3, repeat: -1, repeatDelay: 4, ease: "slow(0.3, 0.7, false)", immediateRender: !0,
          motionPath: { path: "#fish-path", align: "#fish-path", alignOrigin: [0.5, 0.5], autoRotate: !0, start: 0, end: 1 }
        });

        animRef.current = () => { t.kill(); };
        console.log('Island animation started');
      })
      .catch(err => console.error('Failed to load animation scripts:', err));
  }, []);
  ...
  return (
    <div
      ...
      style={{
        ...
        width: 180, height: 200, zIndex: 999,
        pointerEvents: 'none',
    ...
    >
      <div
        dangerouslySetInnerHTML={{ __html: ISLAND_SVG }}
